...
import { postMessageImageCommit } from './mediaServer'
...
/**
 * INIT (WS) → POST /media/message/commit → COMMIT (WS) → FINALIZE (WS).
...
export async function runImageMessageUploadPhases(file, dispatch, ctx) {
  ...
  try {
    ...
    sendMessage(
      JSON.stringify(
        createUploadImageMessageInit(clientId, mimeType, fileSizeBytes),
      ),
    )
    ...
    const putOk = await postMessageImageCommit(uploadId, file, mimeType, initRow.commitUrl ?? null)
    if (!putOk) {
