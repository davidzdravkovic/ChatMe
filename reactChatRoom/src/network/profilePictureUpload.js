...
import { postProfilePictureCommit } from './mediaServer'
...
/**
 * WS INIT → POST /media/profile/commit (body) → WS COMMIT.
 * `approved` from the chat server is the string "true" in JSON (see Stringify.cpp).
 */
export async function runProfilePictureUploadPhases(file, userId) {
  const mimeType = file.type || 'application/octet-stream'
  const fileSizeBytes = file.size

  sendMessage(
    JSON.stringify(
      createUploadProfilePictureInit(mimeType, fileSizeBytes),
    ),
  )
  ...
  const uploadedOk = await postProfilePictureCommit(
    uploadId,
    userId,
    file,
    mimeType,
    initRow.commitUrl ?? null,
  )
  if (!uploadedOk) {
    throw new Error('Profile upload failed (media server POST /media/profile/commit)')
  }

  sendMessage(
    JSON.stringify(
      createUploadProfilePictureCommit(userId, uploadId),
    ),
  )
  ...
  return { uploadId, profileUrl: commitRow.profileUrl ?? null }
}
