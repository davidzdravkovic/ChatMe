...
/** Conversation peer for live MESSAGE_RESPONSE routing (storage, rightChat, list). */
function peerKeyForLiveMessage(message, currentUserId) {
  if (!message) return null
  const senderId = Number(message.SenderId)
  const uid = Number(currentUserId)
  if (Number.isFinite(senderId) && Number.isFinite(uid) && senderId === uid) {
    return normalizePeerLabel(
      message.ReceiverUserName ?? message.receiverUserName ?? null,
    )
  }
  return normalizePeerLabel(message.Sender)
}

function correspondentUserIdForMessage(message, currentUserId) {
  if (!message) return null
  const senderId = Number(message.SenderId)
  const uid = Number(currentUserId)
  if (Number.isFinite(senderId) && Number.isFinite(uid) && senderId === uid) {
    const rid = message.receiver_id ?? message.receiverId
    return rid != null ? Number(rid) : null
  }
  return message.SenderId != null ? Number(message.SenderId) : null
}
...
export function useChatSubscription(
  ...
  function updateTempStorage(data, peerUserName) {
    ...
    appendLastMessageStorage(serverMsg, peerUserName)
  ...
  function setUpIfFirstMessage (otherUserName,chatRoomId,otherUserId) {
          ...
          //Change the current session if the response matches the current session with same username
           if (normalizePeerLabel(otherUserName) !== normalizePeerLabel(env?.peerUserName)) return
  ...
  function appendLastMessageStorage(message, peerUserName) {
  const username =
    normalizePeerLabel(peerUserName) ?? normalizePeerLabel(message?.Sender)
      ...
      switch (msgData.response) {
        ...
        case 'RECENT_CHATROOM_RESPONSE': {
          ...
          const selfProfileUrl =
            recentRows.find((r) => r?.selfProfileUrl != null && r.selfProfileUrl !== '')
              ?.selfProfileUrl ?? null
          if (selfProfileUrl) onOwnProfileUrl?.(selfProfileUrl)
        ...
        case 'FETCH_MESSAGES_RESPONSE': {
             ...
             if (!validSession) return
              ...
              const peerProfileUrl = msgData.data.find((m) => m.profileUrl != null)?.profileUrl ?? null
          ...
          if (activeChatRef.current && !activeChatRef.current.initialFetchDone) {
            ...
            if (chatRoomId) {
              ...
              // View (initial vs last_view) is decided uniformly after this block.
            }
            //NEWCHAT 
            else {
            ...
            if (peerProfileUrl) activeChatRef.current.profileUrl = peerProfileUrl
          ...
          // After reconciliation, decide the view once for any existing-chat fetch that can reach
          // the bottom (initial / anchor jump / append): if the window touches the newest stored
          // tail, merge it and switch to last_view. Skip prepend (older-only) and newChat.
          if (env.state === 'existingChat' && mergeMode !== 'prepend') {
        ...
        case 'FETCH_IMAGES_FOR_CHAT_RESPONSE':
          const env = chatSessionEnvRef.current
          if(!env) return
          const echoed = msgData.data.find((m) => m.chatIdentifier != null)?.chatIdentifier ?? null
          if(echoed !== env.conversationEpoch) return
        ...
        case 'MESSAGE_RESPONSE': {
          const messageRow = msgData.data.find((m) => m.messageId != null) ?? msgData.data[0]
          const chatRoomid = msgData.data.find((m) => m.chatroom_id != null)?.chatroom_id ?? null
          const peerUserName = peerKeyForLiveMessage(messageRow, currentUser?.userId)
          const correspondentUserId = correspondentUserIdForMessage(messageRow, currentUser?.userId)
          let ackPeer = null
          let rightChat = false
          const env = chatSessionEnvRef.current

          if (msgData.response === `MESSAGE_RESPONSE`) {
            if (!peerUserName) break
            rightChat =
              normalizePeerLabel(peerUserName) === normalizePeerLabel(env?.peerUserName) &&
              env?.chatView === `last_view`
            appendLastMessageStorage(messageRow, peerUserName)
            setUpIfFirstMessage(peerUserName, chatRoomid, correspondentUserId)
          }
          else if (msgData.response === `MESSAGE_ACK_RESPONSE`) {
            ackPeer = normalizePeerLabel(
              msgData.data.find((m) => m.ReceiverUserName != null)?.ReceiverUserName ??
                msgData.data.find((m) => m.receiverUserName != null)?.receiverUserName ??
                null,
            )
            {
              ...
              if (
                ...
                normalizePeerLabel(ackPeer) === normalizePeerLabel(envAck.peerUserName)
              ) {
                envAck.chatAcknowledge = true
              }
            }
            updateTempStorage(msgData.data, ackPeer)
            setUpIfFirstMessage(ackPeer, chatRoomid, correspondentUserId)
          }

          const dispatchMeta = {
            payload: msgData,
            peerUserName: peerUserName ?? ackPeer,
            peerUserId: correspondentUserId,
        ...
        case 'REACTION_RESPONSE': {
          ...
          if (
            active?.chatRoomId != null &&
            row.chatroom_id != null &&
            Number(active.chatRoomId) !== Number(row.chatroom_id)
          ) {
            break
          }
          if (
            env?.chatView !== 'last_view' &&
            active?.initialFetchDone !== true
          ) {
            break
          }
          dispatch({
            ...
            payload: msgData,
            activeChatId: active?.chatRoomId ?? null,
          })
          break
        }
        ...
        case 'CHATROOM_ID_RESPONSE': {
          ...
          const roomPeer = normalizePeerLabel(otherUserName)
          if (roomPeer) {
            messageStorageRef.current[roomPeer] ??= []
          }
        ...
        case `SEARCH_QUERY_RESPONSE`: {
          const searcherQueryId =
            msgData.data?.find((m) => m.searcherQueryId != null)?.searcherQueryId ?? null
          if (
            searcherQueryId != null &&
            Number(searcherQueryId) === Number(searchQueryIdRef.current)
          ) {
            dispatch({
              type: 'UPDATE_SEARCH_PANEL',
        ...
        case 'MESSAGE_SEARCH_RESPONSE': {
          ...
          const searchQueryId =
            rows.find((m) => m.searchQueryId != null)?.searchQueryId ?? null
          if (
            searchQueryId == null ||
            Number(searchQueryId) !== Number(messageSearchIdRef?.current)
          ) {
            break
          }
          dispatch({
            type: 'MESSAGE_SEARCH_RESULT',
            payload: rows.filter((m) => m.messageId != null),
          })
          break
        }
