...
import { useLastActiveLabel, lastActiveTooltip, isPeerOnline } from '../../hooks/useLastActiveLabel'
...
const ChatListItem = memo(function ChatListItem({ chat, isActive, onSelect, avatarUrl, typing}) {
  const { correspondentName, lastMessage, online, chatRoomId, lastActiveAt } =
    chat
  ...
  const name =
    correspondentName != null && String(correspondentName).trim() !== ''
      ? String(correspondentName).trim()
      : '?'
  const initials = name
    .split(/\s+/)
    .map((s) => s[0])
  ...
  const isOnline = isPeerOnline(online)
  const statusLabel = useLastActiveLabel(lastActiveAt, isOnline)
  const statusTitle = lastActiveTooltip(lastActiveAt, isOnline)

  return (
    <li className="chat-list-item">
      <button
        type="button"
        className={`chat-list-item-btn ${isActive ? 'chat-list-item-btn--active' : ''} ${isOnline ? 'chat-list-item-btn--online' : 'chat-list-item-btn--offline'}`}
        onClick={() => onSelect?.(chat)}
        title={`${name} · ${statusTitle}`}
        aria-label={`Chat with ${name}, ${statusLabel}`}
      >
        <div
          className={`chat-list-item-avatar ${isOnline ? 'chat-list-item-avatar--online' : 'chat-list-item-avatar--offline'}${
        ...
        <div className="chat-list-item-body">
          ...
          <span className={`chat-list-item-status ${isOnline ? 'chat-list-item-status--online' : 'chat-list-item-status--offline'}`}>
            {statusLabel}
          </span>
          {typingUser ? (
