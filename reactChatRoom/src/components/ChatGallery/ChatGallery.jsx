...
function ChatGallery({ galleryItems, onClose }) {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    Promise.all(
      galleryItems.map(async ({ id, mediaUrl }) => {
        const url = await fetchMessageImage(id, mediaUrl)
        return { id, url }
      }),
    ).then((results) => {
      if (cancelled) {
  ...
  }, [galleryItems?.map((i) => `${i.id}:${i.mediaUrl ?? ''}`).join(',')])
    ...
    <div className="chat-gallery-overlay" role="dialog" aria-modal="true" aria-label="Chat gallery">
      ...
      <div className="chat-gallery-modal">
        ...
        <div className="chat-gallery-grid">
          {imageUrls.length === 0 && galleryItems?.length > 0 && (
            <p className="chat-gallery-loading">Loading images…</p>
          )}
          {imageUrls.length === 0 && (!galleryItems || galleryItems.length === 0) && (
